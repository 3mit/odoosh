...
class hr_payslip(models.Model):
    ...
    # journal_id = fields.Many2one('account.journal', string='Bank', domain=[('type', '=', 'bank')], required=True, readonly=True, states={'draft': [('readonly', False)]}, store=True)
    ...
    def onchange_dateee_end(self):
        if self._context.get('is_payoff'):
            ...
            if self.date_from and self.date_to:
                ...
                fecha_temp = datetime.strptime(str(self.date_to), DEFAULT_SERVER_DATE_FORMAT)
    ...
    def get_years_service(self, fecha_inicio, fecha_fin=None):
        ...
        if fecha_inicio and fecha_fin:
            antiguedad = relativedelta(datetime.strptime(fecha_fin, DEFAULT_SERVER_DATE_FORMAT),
                                                     datetime.strptime(str(fecha_inicio), DEFAULT_SERVER_DATE_FORMAT))
            res = {
    ...
    def get_year_worked_time(self, date_start, date_end=None):
        ...
        if str(date_start) > date_start_year:
    ...
    def compute_sheet_2(self):
        config_obj = self.env['hr.rule.parameter']
        code_liq = config_obj._get_parameter_from_name('hr.payroll.codigos.nomina.liquidacion')
   ...
   #     vacaciones_obj = self.env["hr.payroll.dias.vacaciones"]
        ...
        if self.tiempo_servicio:
            ...
            min_days = int(config_obj._get_parameter_from_name('hr.payroll.vacation.min'))
            max_days = int(config_obj._get_parameter_from_name('hr.payroll.vacation.max'))
        ...
        dias_x_ano = config_obj._get_parameter_from_name('hr.payroll.max.dias.año')
        config_data = False
        is_anticipo = False
        date_range = []
        date_range.append(date(int(self.date_to.year), int(1), 1))
        max_day_util = util_obj.get_last_util_max_days(self.date_to.year)
