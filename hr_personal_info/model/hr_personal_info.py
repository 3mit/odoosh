...
class HrEmployee(models.Model):
    ...
    personal_email = fields.Char('Correo electrónico personal', size=240, required=True)
    ...
    state_id = fields.Many2one('res.country.state',
                               'Estado de nacimiento')  # , domain="[('country_id','=',238)]")    #ESTADO DE NACIMIENTO
    ...
    def _calcular_edad(self):
        for record in self:
            if record.birthday: # Eliminé iteración sobre el booleano: record.birthday[0]
                fecha_fin = date.today()
                fecha_inicio = record.birthday
                fecha_permitida = self._fecha_nacimiento_permitida(str(fecha_inicio), str(fecha_fin)) # Añadí str(fechas)
                if fecha_permitida:
                    antiguedad = relativedelta.relativedelta(datetime.strptime(str(fecha_fin), DEFAULT_SERVER_DATE_FORMAT),
                                                             datetime.strptime(str(fecha_inicio),
                                                                               DEFAULT_SERVER_DATE_FORMAT)) # Añadí str(fechas)
                    years = antiguedad.years
                    record.employee_age = years
                else:
                    raise exceptions.except_orm('¡Advertencia!',
                                                'La fecha de nacimiento introducida "%s" '
                                                'no puede ser mayor a la actual!' % fecha_inicio)
            else:
    ...
    @staticmethod
    def validate_code_postal(valor):
        res = {}
