...
class HrPayslip(models.Model):
    ...
    def get_years_service(self, date_start=None, date_end=None):
        ...
        if date_start and date_end:
            antiguedad = relativedelta.relativedelta(datetime.strptime(str(date_end), DEFAULT_SERVER_DATE_FORMAT),
                                                     datetime.strptime(str(date_start), DEFAULT_SERVER_DATE_FORMAT))
            res = {
    ...
    def get_dias_bono_vacacional(self, tiempo_servicio):
        ...
        max_days = 0
        step_days = 0
        config_param = self.env['hr.rule.parameter']
        if tiempo_servicio:
            min_days = int(config_param._get_parameter_from_name('hr.payroll.vacation.min'))
            max_days = int(config_param._get_parameter_from_name('hr.payroll.vacation.max'))
            # Si hay algun cambio en la ley de los dias que corresponden segun el tiempo de servicio para los dias de
            # bono vacacional, se debe colocar Step_days y agregarlo como parametro
            # step_days = config_param._get_parameter_from_name('hr.payroll.vacation.step'

            # step_days
            pay_days = min_days + ((tiempo_servicio['anios'] - 1) if tiempo_servicio['anios'] > 0 else 0)
            pay_days = pay_days if pay_days < max_days else max_days

            # antiguedad menor a un anio y mayor a 3 meses
            if tiempo_servicio['anios'] == 0 and tiempo_servicio['meses'] >= 3:
            ...
            raise exceptions.except_orm((u'No se han calculado los días de bono vacacional, debido a que\n'
                                         u' el Contrato se encuentra en otro estado que no es en "Proceso" '
                                         u'o Verificar la Fecha de Ingreso del mismo.\n'
    ...
    def calculo_sueldo_promedio(self, employee_id, fecha_desde, meses, tipo_nomina=None, limite=None):
        config_obj = self.env['hr.rule.parameter']
        if 'fideicomiso' in tipo_nomina.lower():
            codes_str = config_obj._get_parameter_from_name('hr.payroll.codigos.salario.integral.fideicomiso')
        elif 'vacaciones' in tipo_nomina.lower():
            codes_str = config_obj._get_parameter_from_name('hr.payroll.codigos.salario.integral.vacaciones')
        elif 'utilidad' in tipo_nomina.lower():
            codes_str = config_obj._get_parameter_from_name('hr.payroll.codigos.salario.integral.utilidades')
        elif 'liquidacion' in tipo_nomina.lower():
            codes_str = config_obj._get_parameter_from_name('hr.payroll.codigos.salario.integral.liquidaciones')
        else:
            codes_str = config_obj._get_parameter_from_name('hr.payroll.codigos.salario.integral')
        # para obtener los conceptos a agregar al salario integrarel monto BRUTO a cobrar
        code = str(codes_str).strip().split(',')
        # para tomar la fecha en que se genera la nomina
        ultimo_sueldo = self.get_amount(code, employee_id.id, fecha_desde, meses, tipo_nomina, limite)
        # ultimo_sueldo = self.get_amount(cr, uid, code, employee_id.id, fecha_desde, 0)
        # para tomar la fecha maxima del rango en que se genera la nomina
        return ultimo_sueldo

    def get_amount(self, code=None, employee_id=None, fecha_desde=None, meses=0, tipo_nomina=None, limite=None):
        config_obj = self.env['hr.rule.parameter']

        code_cest = config_obj._get_parameter_from_name('hr.payroll.codigos.nomina.cestaticket')
        ...
        if rango:
            if 'liquidacion' in tipo_nomina:
                ...
                rango = self.rango_mes_anterior(fecha_desde, 1, 'amount')
                if rango:
                    domain_ps.append(('date_to', '<=', rango[1]))

                payslip_ids_ids = self.search(domain_ps)
                if payslip_ids_ids:
                    payslip_ids = payslip_ids_ids

            if 'vacaciones' in tipo_nomina:
                domain_ps.append(('date_from', '>=', rango[0]))
                rango = self.rango_mes_anterior(fecha_desde, 1, 'amount')
                if rango:
                    domain_ps.append(('date_to', '<=', rango[1]))
            ...
            if payslip_ids:
                ...
                if (tipo_nomina != 'normal') and (self[0].payslip_run_id.struct_id.code != code_cest):
                    raise exceptions.except_orm(('Advertencia!',
                                                 u'No se han confirmado las nóminas correspondientes al mes anterior, '
                                                 u'o puede que no exista.\n \
    ...
    def calculo_alic_bono_vac(self, sueldo_normal, dias_b_v=0):
        config_obj = self.env['hr.rule.parameter']
        alicuota = 0.0
        dias_str = config_obj._get_parameter_from_name('hr.dias.x.mes')
        dias_x_anio = config_obj._get_parameter_from_name('hr.payroll.max.dias.año')
    ...
    def calculo_alic_util(self, sueldo_normal, alicuota_bv):
        ...
        dias_str = config_obj._get_parameter_from_name('hr.dias.x.mes')
        dias_x_anio = config_obj._get_parameter_from_name('hr.payroll.max.dias.año')
        dias_max_str = config_obj._get_parameter_from_name('hr.payroll.max.utili.days.year')
    ...
    def rango_mes_anterior(self, date_base, meses, tipo_trans=None):
        ...
        mes_anterior = datetime.strptime(str(date_base),
                                         DEFAULT_SERVER_DATE_FORMAT) - relativedelta.relativedelta(months=meses)
        ...
        if tipo_trans != 'utilidad':
            ...
            date_range.append(date(int(local_date[0]), int(local_date[1]),
                                   calendar.monthrange(int(local_date[0]), int(local_date[1]))[
                                       1]))  # ultimo dia del mes
        ...
        if hollyday_ids:
            ...
            for h in hollyday_ids:
                if h.date_from == h.date_to:
                    h_from = datetime.strptime(str(h.date_from), DEFAULT_SERVER_DATE_FORMAT)
                    h_to = datetime.strptime(str(h.date_to), DEFAULT_SERVER_DATE_FORMAT)
                    dias = dias + 1
                else:
                    h_from = datetime.strptime(str(h.date_from), DEFAULT_SERVER_DATE_FORMAT)
                    h_to = datetime.strptime(str(h.date_to), DEFAULT_SERVER_DATE_FORMAT)
    ...
    def get_fraccion_dias_util(self, cr, uid, ids, fecha_ingreso, fecha=None):
        ...
        periodo = relativedelta.relativedelta(datetime.strptime(str(fecha), DEFAULT_SERVER_DATE_FORMAT),
                                              datetime.strptime(str(fecha_ingreso), DEFAULT_SERVER_DATE_FORMAT))
        if periodo.years == 0:
...
class HrPayrollDiaVacaciones(models.Model):
...
HrPayrollDiaVacaciones()


class HrPayrollUtilidades(models.Model):
    ...
    def get_last_util_max_days(self, year):
        days = 0
        if year:
            util_id = self.search([('utilidades_name', '=', year)])
            # , limit=1, order='utilidades_name, write_date desc')
        else:
            util_id = self.search([])
            # , limit=1, order='utilidades_name, write_date desc')
