...
class Contract(models.Model):
 ...
 #   salary_assignment_check = fields.Boolean(string='Salary Assignment')
  #  salary_assignment_value = fields.Integer(string='Salary Assignment Value')
    ...
    # @api.multi No se necesita en odoo 13
    def _validate_value_digits(self, values):
        lista = ['sundays_value', 'saturdays_value', 'holidays_value', 'days_of_salary_pending_value',
                 'ausencias_ded_value', 'unpaid_permit_days_value']
        if values:
            for b in values:
                for c in lista:
                    if c == b:
    ...
    def _restore_all_fields(self, ids):
        ...
        for contract in self.browse(contract_id[0]):
            ...
            if contract.saturdays_check:
                contract_fields.update({'saturdays_check': False, 'saturdays_value': '0'})
...
class HrPayslipRun(models.Model):
    ...
    #     return res
    # @api.multi no disponible en odoo 13
